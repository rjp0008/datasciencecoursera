corr <- function(directory,threshold = 0){
  ...
  for(file in files){
    ...
    nitrate <- (dataTable$nitrate[(!is.na(dataTable$nitrate)) & (!is.na(dataTable$sulfate))])
    if(length((nitrate))>threshold){
      sulfateVector <-(dataTable$sulfate[(!is.na(dataTable$nitrate)) & (!is.na(dataTable$sulfate))])
